...
window.__orgValidate = function (cid, print) {
  var r = orgLayoutForTest(cid, orgDefaultSettings(), print); if (!r) return null;
  var g = r.g, L = r.L, V = L.V, issues = [];
  function inter(a, b, pad) { pad = pad || 0; return a.l < b.r - pad && b.l < a.r - pad && a.t < b.b - pad && b.t < a.b - pad; }
  var boxes = Object.keys(g.nodes).map(function (k) { var v = V[k]; return { k: k, l: v.x - v.w / 2, r: v.x + v.w / 2, t: v.top, b: v.bottom, kind: 'card' }; });
  L.inv.forEach(function (s) { s.items.forEach(function (it, i) { boxes.push({ k: 'inv:' + s.holder + ':' + i, holder: s.holder, l: it.x, r: it.x + 150, t: it.y, b: it.y + 24, kind: 'chip' }); }); });
  ...
  lines.forEach(function (ln) {
    ...
    boxes.forEach(function (b) { if (inter(lb, b, 1)) issues.push('line-through ' + b.k + ' by ' + ln.k); });
  });
  for (var a = 0; a < lines.length; a++) for (var c = a + 1; c < lines.length; c++) {
    var p = lines[a], q = lines[c]; if (p.k === q.k || p.inv || q.inv) continue;
    var ks = function (s) { return s.split('|').filter(Boolean); };
    var pk = ks(p.k), qk = ks(q.k), sub = pk.every(function (x) { return qk.indexOf(x) !== -1; }) || qk.every(function (x) { return pk.indexOf(x) !== -1; });
    if (sub) continue;
    var ph = p.y1 === p.y2, qh = q.y1 === q.y2;
    if (ph && qh && Math.abs(p.y1 - q.y1) < 2 && Math.min(p.x2, q.x2) - Math.max(p.x1, q.x1) > 1) issues.push('overlap-h ' + p.k + ' / ' + q.k);
    if (!ph && !qh && Math.abs(p.x1 - q.x1) < 2 && Math.min(p.y2, q.y2) - Math.max(p.y1, q.y1) > 1) issues.push('overlap-v ' + p.k + ' / ' + q.k);
  }
  // text: % labels and tags
  var texts = L.labels.map(function (t) { return { l: t.x, r: t.x + Math.min(t.text.length * 6.6, t.maxW), t: t.y - 11, b: t.y + 3, k: 'lbl' + t.k }; });
  L.tags.forEach(function (t) { var txt = (t.a.pct != null ? '100.00% ' : '') + t.a.name + ' ↗'; texts.push({ l: t.x + 17, r: t.x + 17 + Math.min(txt.length * 5.9, t.maxW), t: t.y - 7, b: t.y + 7, k: 'tag' + t.k }); });
  texts.forEach(function (tx, i) {
    boxes.forEach(function (b) { if (inter(tx, b, 0.5)) issues.push('text-on-box ' + tx.k + ' ' + b.k); });
    lines.forEach(function (ln) { var lb = { l: ln.x1 - 0.5, r: ln.x2 + 0.5, t: ln.y1 - 0.5, b: ln.y2 + 0.5 }; if (inter(tx, lb, 0)) issues.push('text-on-line ' + tx.k + ' / ' + ln.k); });
    for (var j2 = i + 1; j2 < texts.length; j2++) if (inter(tx, texts[j2], 0.5)) issues.push('text-overlap ' + tx.k + ' / ' + texts[j2].k);
  });
  // completeness
  var byId = {}; data.companies.forEach(function (c) { byId[c.id] = c; });
  var focalLiq = byId[cid] && byId[cid].status === 'liquidated';
  Object.keys(g.nodes).forEach(function (k) {
    var n = g.nodes[k]; if (!n.coId || n.collapsedUp) return;
    (byId[n.refId].shareholders || []).forEach(function (s) {
      if (!s.person || (s.type === 'company' && s.person === n.refId)) return;
      if (s.type === 'company' && byId[s.person] && byId[s.person].status === 'liquidated' && !focalLiq) return;
      var ok = s.type === 'company' ? 'co:' + s.person : 'ind:' + s.person;
      var drawn = g.edges.some(function (e) { return e.from === ok && e.to === k; });
      var tagged = n.also.some(function (x) { return x.key === ok; });
      if (!drawn && !tagged) issues.push('missing-owner ' + ok + ' -> ' + k);
    });
    if ((n.role === 'down' || n.role === 'focal') && !n.collapsedDown) {
      var want = (data.investments || []).filter(function (x) { return invCoIds(x).indexOf(n.refId) !== -1; }).length;
      if (n.invs.length !== want) issues.push('investments ' + k + ' ' + n.invs.length + '/' + want);
      if (print) { var s2 = L.inv.find(function (x) { return x.holder === k; }); var shown = s2 ? s2.items.length : 0; if (shown !== want) issues.push('inv-drawn ' + k + ' ' + shown + '/' + want); }
    }
  });
  return { n: Object.keys(g.nodes).length, w: L.width, h: L.height, issues: issues };
};
window.__orgLineStats = function (cid) {
  var d = document.createElement('div'); d.innerHTML = buildFilteredOrgChart(cid, orgDefaultSettings());
  var total = 0, maxH = 0, n = 0;
  d.querySelectorAll('path.org2-e').forEach(function (p) {
    if (p.closest('.org2-tag')) return;
    n++; var cmds = (p.getAttribute('d') || '').match(/[MVHL][^MVHL]*/g) || [], x = 0, y = 0;
    cmds.forEach(function (c) { var v = c.slice(1).trim().split(/[ ,]+/).map(Number);
      if (c[0] === 'M') { x = v[0]; y = v[1]; } else if (c[0] === 'V') { total += Math.abs(v[0] - y); y = v[0]; } else if (c[0] === 'H') { var len = Math.abs(v[0] - x); total += len; maxH = Math.max(maxH, len); x = v[0]; }
      else { var dx = Math.abs(v[0] - x), dy = Math.abs(v[1] - y); total += dx + dy; maxH = Math.max(maxH, dx); x = v[0]; y = v[1]; } });
  });
  return { paths: n, len: Math.round(total), maxH: Math.round(maxH), cards: d.querySelectorAll('.org2-card').length };
};
window.__orgValidateAll = function (print) {
  var res = { total: 0, bad: [], big: [] };
  data.companies.forEach(function (c) {
    var r; try { r = __orgValidate(c.id, print); } catch (e) { res.bad.push(c.name + ': EXC ' + e.message + ' ' + (e.stack || '').split('\n')[1]); return; }
    if (!r) return; res.total++;
    if (r.issues.length) res.bad.push(c.name + ': ' + r.issues.length + ' — ' + r.issues.slice(0, 3).join(' ; '));
    res.big.push([r.n, r.w, r.h, c.name]);
  });
  res.big.sort(function (a, b) { return b[1] - a[1]; }); res.big = res.big.slice(0, 6).map(function (x) { return x.join(' / '); });
  return res;
};
window.__orgStatsAll = function () { var out = {}; data.companies.forEach(function (c) { try { out[c.name] = __orgLineStats(c.id); } catch (e) { out[c.name] = { err: e.message }; } }); return out; };
